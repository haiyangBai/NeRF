...
from dataset.dataset import get_nerf_datasets, trivial_collate
from models.nerf_renderer import RadianceFieldRenderer, visualize_nerf_outputs
...
def main(cfg: DictConfig):
    ...
    if stats is None:
        stats = Stats(["loss", "mse_coarse", "mse_fine", "psnr_coarse", "psnr_fine", "sec/it"])
    
    def lr_lambda(epoch):
        return cfg.optimizer.lr_scheduler_gamma ** (epoch / cfg.optimizer.lr_scheduler_step_size)
    ...
    if cfg.visualization.visdom:
        viz = Visdom(server=cfg.visualization.visdom_server,port=cfg.visualization.visdom_port,use_incoming_socket=False)
    else:
        viz = None

    # Load the training/validation data.
    train_dataset, val_dataset, _ = get_nerf_datasets(dataset_name=cfg.data.dataset_name, image_size=cfg.data.image_size)
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=0, collate_fn=trivial_collate,)

    # The validation dataloader is just an endless stream of random samples.
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=1, num_workers=0, collate_fn=trivial_collate,
        sampler=torch.utils.data.RandomSampler(val_dataset, replacement=True, num_samples=cfg.optimizer.max_epochs))
    ...
    for epoch in range(start_epoch, cfg.optimizer.max_epochs):
        ...
        for iteration, batch in enumerate(train_dataloader):
            ...
            nerf_out, metrics = model(camera_idx if cfg.data.precache_rays else None, camera, image)
            ...
            stats.update({"loss": float(loss), **metrics}, stat_set="train")
        ...
        if epoch % cfg.validation_epoch_interval == 0 and epoch > 0:
            ...
            model.eval()
            with torch.no_grad():
                val_nerf_out, val_metrics = model(camera_idx if cfg.data.precache_rays else None, val_camera, val_image)
            ...
            if viz is not None:
                ...
                stats.plot_stats(viz=viz, visdom_env=cfg.visualization.visdom_env, plot_file=None)
                # Visualize the intermediate results.
                visualize_nerf_outputs(val_nerf_out, visuals_cache, viz, cfg.visualization.visdom_env)
